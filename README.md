...
17. 在研究页把模拟净值账本、不可覆盖的模拟日报和当前用户日志投影为逐日摘要、ISO 周度复盘、自然月复盘与历史持仓数量快照，并可将日/周证据按用户和模拟账期追加为不可变 digest 修订链；旧账期只能在只读浏览器中核验，不能替换或复活活动账户。
...
21. 在当前登录账户下把规则告警和失败扫描幂等汇总为本地通知，保留来源与证据指纹，并用追加式动作记录未读、已读和归档；可选 HTTPS HMAC Webhook、SMTP 邮件和交互式 Windows Toast 都有独立投递审计，失败不会改变本地收件箱或取得交易权限。
...
28. 将验证通过的日报/周报 digest 单独备份到私有 R2 命名空间，恢复只进入新暂存目录并逐条复核 schema、账期、配置、修订链和指纹，永不覆盖活动状态。
...
推荐普通用户从 [GitHub Releases](https://github.com/Shiraikuroko123/ai-trade/releases/tag/v1.0.0) 安装当前发行版。下面的命令创建隔离环境和独立工作目录，不需要 Git：

```powershell
New-Item -ItemType Directory -Force .\AI-Trade-v1.0.0 | Out-Null
Set-Location .\AI-Trade-v1.0.0
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install "https://github.com/Shiraikuroko123/ai-trade/releases/download/v1.0.0/ai_trade-1.0.0-py3-none-any.whl"
...
如果提交请求在券商 I/O 后超时或返回内容无法校验，订单会停留在 `PENDING_SUBMIT`。交易页会显示“提交结果未确认”和数量计数；这表示本地只保存了提交意图，不能推断订单已拒绝或已成交。请在券商端按客户端订单号查询，再让适配器写入对应快照；确认前不要重复提交、删除账本行或复用客户端订单号，系统不会自动重试。
...
正式现金与持仓对账记录位于本地 `state/broker_reconciliation.csv`。新的合格记录以 `v3_` 内容指纹同时绑定适配器、账户、日期、配置、非负现金、双方完整规范化持仓与问题明细；写入和读取时都会从现金及持仓重新计算问题，调用方不能通过漏报问题把不一致记录成“干净”。读取时会先校验整本账本，再统计当前账户的连续干净会话。并发写入会串行化，更新通过同目录临时文件落盘后原子替换；同一次观察可以幂等重试，替换失败会保留上一份完整文件。若同一账户、日期和配置出现不同内容，冲突也会被保留并关闭晋级资格，不能靠保留旧的“干净”行继续累计。

旧版 24 位 ID 只绑定记录身份，`v2_` 只绑定现金和问题明细；两者都没有绑定实际比较的双方持仓。升级后它们仍可读取和幂等复查，但不再计入默认 20 次晋级门槛，也不会自动迁移或与 `v3_` 混写。需要由未来具备合格沙箱对账能力的适配器在归档旧账本后重新积累未来 `v3_` 会话；当前 QMT 只读插件不会写入这类证据。内容指纹不是数字签名，不能替代本地文件权限、备份和券商侧正式结单。
...
`v1.0.0` 的左侧 **市场情报** 视图包含九个独立证据集：历史分钟行情、当前估值与股票历史分位、股票时点基本面、官方披露、第三方新闻与公告、Level-1 五档盘口、龙虎榜、市场宽度与板块排名、板块资金流。每次刷新都校验日期或截止时间、字段、有限数值、来源响应指纹和规范化证据指纹，再在各自的 `state/` 子目录追加不可覆盖的本地 revision。相同规范化证据会幂等复用，证据改变会追加带 `supersedes` 的新版本，失败或取消不会覆盖上一份完整快照。
...
`v1.0.0` 会为每份新分析生成确定性的 **视角冲突审计** 和 **多空裁判账本**。多头、空头、裁判分别保存结构化状态、证据引用、Token、费用与失败码；模型模式下三个角色各自经过预算、并发、重试和用户隔离缓存，单个角色失败不会抹去其他角色，裁判仍可在已验证的本地回退材料上继续整理。本地模式不发送模型请求。多头与空头只能登记论点和反证，裁判只能整理一致点、冲突点和未决问题，不能投票、输出订单/仓位/目标价/止损、改变确定性结论或放宽任何门禁。每份新记录还会在保存和历史读取时核对其公开调用摘要与用户隔离的不可变调用文件；缺失、篡改、跨用户或字段不匹配的已绑定模型记录不会参与历史和下一次比较。股票基本面角色仍只读取最终 K 线交易日已经落盘的不可变记录；数据稀疏、多空冲突或 Tushare 字段核验冲突时明确弃权，ETF 和 `provisional` 估值不进入结论。
...
本节的持久化日报/周报、生成接口和计划任务已包含在 `v1.0.0` wheel 中。
...
默认日志目录为 Git 忽略的 `state/research_journal/users/<owner-sha256>/entries/`，不进入发行包，也不上传 Cloudflare R2。R2 行情备份只允许 `data/cache` CSV 与行情 manifest；独立的 digest 云备份只允许已验证的日报/周报 revision，仍不会包含研究日志。

同页的“收盘归档”会在读取时严格校验并组合当前模拟账户的 `state/paper_equity.csv`、`reports/paper_YYYYMMDD.json` 与当前登录用户的研究日志，生成逐日摘要、ISO 周度汇总、自然月汇总和持仓数量快照。权益、现金、持仓、目标权重或行情快照 ID 在日报与净值账本间不一致时会显示“证据不一致”；缺少日报、日报未绑定账本、仅有日志和部分可用也都有独立文字状态。缺失值不会补零，历史快照只披露账本记录的数量，不按当前或历史价格推算市值。
...
研究页可选择 `state/archive/` 中通过校验的旧模拟账期。旧账期读取只使用归档账本、日报和匹配的 digest 命名空间；不会复制、合并、恢复或激活旧状态。研究日志目前没有 `account_id` 账期字段，因此旧账期投影明确排除同日人工日志，避免把活动账期笔记误归入历史账户。

配置私有 R2 后，可显式备份、列出和恢复当前本地所有者的 digest：
...
18:00 前向数据证据、18:10 模拟盘、18:20 监控和 18:30 全用户归档是四个独立的错峰计划任务，不是带依赖关系的流水线，也不保证前一任务已在后一任务启动前完成。行情快照锁会阻止两个进程同时发布，模拟盘在前向刷新完成后可复用 30 分钟内的同截止日快照，避免紧接着再下载一遍。前向证据任务只有在行情刷新成功后才创建 Feature/Label 快照；归档任务使用 `--all-profiles --trigger scheduled`，只消费运行时可安全读取的最近已完成证据，不会刷新行情或取得交易权限。`scheduled` 是操作者可传入的审计标签和内置 runner 约定，不是经过认证的任务来源证明。安装、日志复核和卸载方式见 [前向证据手册](docs/FORWARD_EVIDENCE.md) 与 [日报/周报归档](docs/RESEARCH_DIGESTS.md)。
...
命令行直接执行 `paper-run` 时仍会强制刷新完整行情。工作台按钮和每日任务使用
`paper-run --refresh-if-needed`：先校验活动模拟账户与当前策略、风控、成本和证券主数据指纹；配置漂移时在联网前失败关闭。账户有效后，已校验快照达到当前完整交易日时直接复用；若刚刚完成过一次针对同一截止日的完整刷新尝试，即使少数证券只能使用近期已验证缓存，也会在 30 分钟内复用该原子快照，避免连续下载两遍。清单缺失、哈希不一致、截止日不符或刷新尝试超过 30 分钟时仍会重新联网。
...
`cloud-backup` 只打包当前已校验的 `data/cache` CSV 与 `manifest.json`；`cloud-digest-backup` 使用另一条严格白名单，只打包当前本地所有者和活动模拟账期下已验证的日报/周报 revision。两者都不会上传研究日志、模拟账户、助理历史、内测账号、券商凭据或任何实盘授权。R2 endpoint、bucket、安装命名空间、对象 key 和访问密钥不会返回网页或写入报告；恢复命令只写新的 `local/` 暂存目录，不会覆盖活动缓存或活动 digest。完整口径、命令、权限建议、轮换和恢复检查见 [云端行情快照](docs/CLOUD_STORAGE.md)。
...
- 默认先串行请求东方财富；主源失败时使用腾讯财经日线回退，若刷新级传输熔断已打开，后续标的会跳过重复的主源请求。请求前复权而腾讯只返回 `day` 时，原始序列默认不可用；只有新浪 qfq 因子严格证明调整因子恒等，且新浪与腾讯核对满足显式的交易日覆盖、单 tick 价格差异、可用成交量和缺失数量上限时，才把该证券记录为有独立证据的“原始价等价于前复权”。全量重建核对完整请求历史；增量刷新只重新证明本次下载区间及最近 320 个缓存交易日，较早历史继续受已发布 manifest、文件 SHA-256 和新旧重叠一致性约束。来源元数据会保存证据范围、因子/参考响应哈希、覆盖率、两侧缺失日期、价格差异和缺失成交量日期；任一硬上限失败即整支证券失败关闭。三个公开接口都不构成交易所或授权数据保证。
...
- 默认刷新后用 Yahoo Finance 核对最近 5 个完整交易日；若本次文件实际来自腾讯回退，仍使用 Yahoo 作为独立参考，不会把腾讯与自身比较后标记通过。Yahoo 的成交额不可用，因此审计只比较 OHLCV。若 Yahoo 日成交量恰好发生 `2^32` 股整数回绕，只有差值在 1 手内严格吻合回绕模数且修正后仍通过原有 10% 容差时才归一化，并把原值、修正值和回绕次数写入签名审计。结果、容差和逐证券偏差位于 `manifest.json -> cross_source_check`，详见 [跨源日线审计](docs/CROSS_SOURCE_AUDIT.md)。
...
截至 2026-07-28，活动 `data/cache/manifest.json` 已原子发布配置内全部 47 只 ETF 和 47 个 CSV，共同完成日为 2026-07-28；逐文件行数、SHA-256 和最后交易日均已校验，manifest SHA-256 为 `b66feec29f08577b26d997ba7e79123f3f04504bb93804b6a2d72f1c5bca0822`。本轮 33 个文件按最近 320 个缓存交易日的有界重叠增量刷新、14 个文件抓取全历史，最终 47 条实际路由均为腾讯网络回退；其中 10 个只返回原始 `day` 的增量标的绑定新浪恒等 qfq 因子及重叠窗口 OHLC/可用成交量等价证据，2 个全量标的绑定完整请求历史证据，其余返回腾讯 `qfqday`。`universe-verify` 最近结果为 47/47 verified、0 missing cache、0 dangerous issues。

Yahoo 对最近 5 个完整交易日的独立核验为 47/47 matched、0 mismatch，状态为 `passed / independent_confirmed`，审计 SHA-256 为 `4122b2baf2174fbf8da905d2d2fdefab51a779517dbc4c72e68180304d15febd`。`513180` 和 `588000` 的 2026-07-28 Yahoo 成交量各发生一次可精确证明的 `2^32` 股回绕，审计保留原始余数、归一化值和回绕次数；修正后的相对偏差分别低于 `8e-9` 和 `5e-9`，OHLC 未做任何特殊处理。该通过只说明近期公开来源一致，不构成授权、交易所认证或 SLA，也不会提升交易权限。

同日 `feature-forward-run` 生成合格的未来积累快照 `fs_5fb3d2c00340f4defe4b66b8a5a9324f`：47 行、7 个默认因子、实际来源 `tencent`、`genuine_pit=true`；重复运行复用同一 ID，没有追加修订。当前有 2 份合格未来快照，5/20/60 日共 6 个标签均为 pending。针对 2026-07-24、但到更晚日期才创建的旧记录仍保留为不可变审计，按严格时点规则不属于 genuine PIT，也不能进入部署训练。
...
- `state/research_digests/users/<owner-sha256>/accounts/<account-epoch-sha256>/digests/`：按用户和模拟账期隔离的日报/周报不可变 revision 链；重复证据幂等复用，变化证据通过 `supersedes` 追加。活动接口仍绑定当前账期，研究页另提供严格只读的旧账期浏览器，不能把旧状态写回或切换为活动账户。
...
“刷新行情”或模拟任务结束后，工作台会自动重读总览、行情、组合、交易、风险、数据和系统等受影响页面。系统页打开某个任务详情后，运行中的标准输出与错误日志会持续更新，不必等到任务结束才判断是否仍有进展。
...
若工作台检测到已有账户文件但配置指纹已变化，组合页会明确显示“需要开启新账期”，不会再把它误报成“尚未建立”并反复提交必然失败的初始化任务。网页不会代替用户执行 `--overwrite`；旧账期在显式执行上述命令前保持原样。
...
日志位于 `logs/scheduled_paper.log`。runner 从启动和每个子命令开始时即追加 UTF-8 时间戳，保留最多 5 份轮换日志；旧版 UTF-16 日志会在首次新运行时自动归档。计划任务隐藏窗口、拒绝同名并发实例、允许电池供电运行，并以 35 分钟为任务级硬上限。卸载任务：
...
监控任务调用一次性 `monitor-scan --all-profiles`，不会启动常驻服务或修改策略、模拟账本和券商权限。本地收件箱会把新告警和失败扫描按来源 ID 幂等登记；可选外部渠道包括 HTTPS HMAC Webhook、SMTP SSL/STARTTLS 邮件和交互式 Windows Toast。源码用户可运行 `scripts/configure_notifications.ps1` 配置邮件或 Toast；Toast 需要当前 Windows 用户的交互会话，容器只支持邮件。任一渠道失败都不会改变本地扫描结果。完整命令、变量、重试语义和审计边界见 [监控与告警运维](docs/MONITORING.md)。
...
.\.venv\Scripts\python.exe -m build --outdir .\dist\release-1.0.0
.\.venv\Scripts\python.exe .\scripts\verify_distribution.py .\dist\release-1.0.0
.\.venv\Scripts\python.exe -m ai_trade.cli doctor
.\.venv\Scripts\python.exe -m ai_trade.cli validate
