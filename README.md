...
新订单事件使用 `v2_` 前缀的整行规范化内容指纹，可在读取时发现事件字段被意外改写。旧版本生成的 24 位事件 ID 仍可直接读取和幂等重试，历史行不会被自动迁移；旧账本中的 `10` 与新版规范化的 `10.0` 按相同数值处理。该指纹用于本地完整性检查，不是数字签名，也不能替代文件权限、备份或券商侧对账。
...
- `state/broker_fills.csv`：带唯一成交号的标准券商成交；数量和加权均价必须与订单快照复算一致。
