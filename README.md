...
`paper-run --refresh-if-needed`：已校验快照达到当前完整交易日时直接复用；若刚刚完成过一次针对同一截止日的完整刷新尝试，即使少数证券只能使用近期已验证缓存，也会在 30 分钟内复用该原子快照，避免连续下载两遍。清单缺失、哈希不一致、截止日不符或刷新尝试超过 30 分钟时仍会重新联网。
