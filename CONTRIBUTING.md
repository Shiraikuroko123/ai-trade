...
AI Trade `v0.16.0` is the current public release baseline. The project prioritizes timing correctness, reproducibility, provenance, and loss controls over higher backtest returns or a larger feature count.
...
python -m build --outdir dist/release-0.16.0
python -m build adapters/qmt --outdir qmt-dist
python scripts/verify_distribution.py dist/release-0.16.0
```
