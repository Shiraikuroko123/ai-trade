...
The integrated local acceptance snapshot updated on 2026-07-28 has 925 passing unittest cases, 76.3% branch coverage, a clean Ruff run, and zero Mypy errors across 50 research-core files. Compileall, JavaScript and PowerShell syntax checks, eight focused QMT adapter tests, and `git diff --check` also pass. Isolated wheel/sdist builds for the main and QMT packages and main-package distribution verification pass as well. After normalizing the Windows workspace-path assertion, commit `e6862b4` passed [CI run 30319817678](https://github.com/Shiraikuroko123/ai-trade/actions/runs/30319817678) on 2026-07-28, including Ubuntu and Windows on Python 3.10/3.12, the quality job, PowerShell/bootstrap, and package smoke tests. The earlier failed `adf8456` run and the older 22-file/71-test/77.2% snapshot are historical and must not be cited as the current quality result.
