...
class DistributionVerificationTests(unittest.TestCase):
    ...
    def test_market_intelligence_release_surface_is_required(self):
        for module in (
            ...
            with self.subTest(module=module):
                self.assertIn(f"ai_trade/data/{module}", WHEEL_REQUIRED)
                self.assertIn(f"src/ai_trade/data/{module}", SDIST_REQUIRED)
        self.assertIn("docs/MARKET_INTELLIGENCE.md", SDIST_REQUIRED)
        self.assertIn("docs/MARKET_BREADTH.md", SDIST_REQUIRED)
        self.assertIn("docs/CAPITAL_FLOW.md", SDIST_REQUIRED)

    def test_research_journal_release_surface_is_required_in_both_artifacts(self):
        self.assertIn("ai_trade/research_archive.py", WHEEL_REQUIRED)
        self.assertIn("src/ai_trade/research_archive.py", SDIST_REQUIRED)
    ...
    def test_research_digest_release_surface_is_required(self):
        for module in (
            "research_digest.py",
            "research_digest_cloud.py",
            "research_epochs.py",
        ):
            self.assertIn(f"ai_trade/{module}", WHEEL_REQUIRED)
            self.assertIn(f"src/ai_trade/{module}", SDIST_REQUIRED)
    ...
    def test_notification_channels_are_required_in_both_artifacts(self):
        self.assertIn("ai_trade/notification_channels.py", WHEEL_REQUIRED)
        self.assertIn("src/ai_trade/notification_channels.py", SDIST_REQUIRED)
        self.assertIn("scripts/configure_notifications.ps1", SDIST_REQUIRED)
