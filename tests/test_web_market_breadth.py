...
from ai_trade.data.market_breadth import MarketBreadthQuery
from ai_trade.web.server import (
    DashboardServer,
    _handler_factory,
    _parse_market_breadth_query,
)
from ai_trade.web.service import DashboardService


class MarketBreadthQueryTests(unittest.TestCase):
    def test_empty_and_filtered_queries_are_bounded(self):
        self.assertEqual(_parse_market_breadth_query(""), MarketBreadthQuery())
        self.assertEqual(
            _parse_market_breadth_query(
                "date=2026-07-17&q=%E7%94%B5%E5%8A%9B&sort=advance_share"
    ...
    def test_parser_rejects_ambiguous_and_unbounded_values(self):
        for query in (
            ...
            with self.subTest(query=query), self.assertRaises(ValueError):
                _parse_market_breadth_query(query)


class DashboardMarketBreadthServiceTests(unittest.TestCase):
    def test_service_passes_cutoff_and_requests_full_revision_history(self):
        config = SimpleNamespace(cache_dir=Path("unused"))
        query = MarketBreadthQuery(q="银行", limit=10)
        service = DashboardService(config)
        service.market = Mock(
            return_value=SimpleNamespace(completed_through=date(2026, 7, 17))
        )
        store = Mock()
        store.list.return_value = {"available": True, "sectors": []}

        with patch("ai_trade.web.service.MarketBreadthStore", return_value=store):
            result = service.market_breadth(query)

        self.assertTrue(result["available"])
        self.assertRegex(result["generated_at"], r"^\d{4}-\d{2}-\d{2}T")
        store.list.assert_called_once_with(
            MarketBreadthQuery(q="银行", limit=10, include_revisions=True),
    ...
    def test_snapshot_remains_readable_without_ordinary_market_cache(self):
        ...
        self.assertTrue(result["available"])
        store.list.assert_called_once_with(
            MarketBreadthQuery(include_revisions=True),
            completed_session_cutoff=None,
        )


class DashboardMarketBreadthHttpTests(unittest.TestCase):
    def test_real_service_get_is_local_and_network_free(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch("ai_trade.web.service.MarketBreadthStore", return_value=store),
                patch(
                ...
                status, payload = _request_json(port, "/api/market-breadth")
    ...
    def test_get_supports_filters_and_rejects_unknown_parameters(self):
        ...
        with _RunningServer(service) as port:
            status, payload = _request_json(
                port,
                "/api/market-breadth?date=2026-07-17&q=%E9%93%B6%E8%A1%8C"
                "&sort=market_cap&direction=desc&limit=10",
            )
            self.assertEqual(status, 200)
            ...
            for query in ("unknown=1", "sort=name&sort=change_pct", "limit=501"):
                bad_status, bad_payload = _request_json(
                    port, f"/api/market-breadth?{query}"
                )
...
class _HttpService:
    ...
    def __init__(self):
        self.queries: list[MarketBreadthQuery] = []

    def market_breadth(self, query: MarketBreadthQuery):
