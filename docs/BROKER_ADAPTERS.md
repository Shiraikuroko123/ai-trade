...
AI Trade `v0.18.1` defines and tests a broker boundary. The core wheel intentionally ships no broker-specific implementation. The source repository now includes an independently installable QMT read-only observation plugin under `adapters/qmt`; it cannot place or cancel orders. Installing any adapter does not unlock live trading.
