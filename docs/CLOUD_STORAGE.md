...
行情快照与 research digest 使用彼此独立的严格白名单。行情导出器只能读取上述
`data/cache` 成员；digest 导出器只能读取一个已校验的用户、模拟账户账期和配置
绑定下的 `state/research_digests/.../digests/` 修订文件。它们不会因为共享同一 R2
配置而扩大彼此的读取范围。
...
A/B 账本从当前公开发行版 `v1.0.0` 在当前工作区首次启用云功能时开始，无法补记首次使用前的操作。它不包含 Cloudflare 控制台、其他应用、其他设备、其他 AI Trade 工作区或 SDK 内部重试发出的请求。因此页面明确标记为“本机观测”，不能用它替代 Cloudflare Analytics 或账单。
...
日报和周报 digest 可使用同一组私有 R2 凭据，但保存在独立的
`research-digests` 对象命名空间。先在本地生成并校验 digest，再显式执行：
...
wheel 用户将命令前缀替换为 `ai-trade`。备份只包含当前登录用户和当前活动模拟
账期的不可变 daily/weekly revision 链；不会包含模拟账本、研究日志、会话、通知、
模型调用记录或凭据。创建快照时会再次验证 owner、account、configuration 绑定，
并要求源文件在两次读取之间保持稳定。

恢复前会验证 ZIP 成员路径、大小、逐文件哈希、严格 manifest、原始 digest schema、
连续 revision、`supersedes` 链及 ID/指纹唯一性。即使攻击者同步修改 ZIP 内容和
manifest 哈希，语义不一致的修订链也会被拒绝。默认恢复位置是
`local/cloud-digest-restore/<snapshot-id>/`；目标必须是尚不存在的新目录，结果返回
`active_state_unchanged: true`，且永远不会覆盖或激活
`state/research_digests/`。采用恢复数据必须经过独立人工核验，本版没有自动导入
命令。

存储页的容量清点只统计行情快照命名空间，不把 digest 对象并入其容量数字；
`put_object`、`list_objects_v2`、`head_object` 和 `get_object` 等 digest 请求仍会进入
同一份本机 A/B 操作观察账本。这些统计仍不是 Cloudflare 官方账单。
