...
A/B 账本从当前公开发行版 `v0.13.0` 在当前工作区首次启用云功能时开始，无法补记首次使用前的操作。它不包含 Cloudflare 控制台、其他应用、其他设备、其他 AI Trade 工作区或 SDK 内部重试发出的请求。因此页面明确标记为“本机观测”，不能用它替代 Cloudflare Analytics 或账单。
