...
Docker 方案用于从源码启动当前 `main` 工作台。它不改变策略、回测、风控、
模拟账本或券商权限，也不会把当前未发布能力伪装成 `v0.12.1` wheel。
...
只有容器显式收到的环境变量才可见。需要模型增强或 R2 时，在本地
`.env.docker` 填写对应变量再重建容器；本地规则无需 AI Key，纯本地存储无需
R2 凭据。
