...
Formal sandbox reconciliation is a separate authority-bearing ledger. New rows bind adapter, account, date, configuration, cash values, issue count, and canonical issue details into a `v2_` content fingerprint. The auditor validates the exact schema and every row before selecting the active account; duplicate logical sessions, malformed rows, reordered dates, and fingerprint mismatches fail closed. Updates use the same in-process/operating-system locking and atomic same-directory publication as lifecycle ledgers. Legacy 24-character identity-only rows remain readable for operational continuity but are excluded from the consecutive clean-session gate because they cannot prove that their cash and issue content was unchanged.
