...
When enabled, an independent daily-bar audit runs after publication and only
...
The workstation has two explicit local access profiles. Beta mode protects every data API, job, and report with a password-authenticated in-memory session and a session-bound CSRF token. Owner-local mode deliberately bypasses that login for one trusted loopback-only process; it is a convenience profile, not a remotely enforceable license. Native processes remain loopback-only. The Docker path may bind `0.0.0.0` inside its own network namespace only when an explicit container flag and beta authentication are both active; the supplied Compose mapping remains host-loopback-only, and browser Host validation still rejects non-loopback names. Owner-local plus container binding fails closed.
...
- `web/`: loopback-first authenticated HTTP server with an explicit authenticated container-network exception, non-mutating market-chart projection, background job manager, dashboard service, and packaged static application with pinned local KLineChart assets.
