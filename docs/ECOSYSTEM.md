...
| Fundamental role | Evidence available, assistant integration not implemented | Stock-only point-in-time fundamentals are available for human review, but the assistant does not yet consume or cite that store. Its fundamental view therefore remains explicitly `UNAVAILABLE`. |
...
| Point-in-time company fundamentals | Partial, stock-only evidence implemented | EPS, revenue, parent net profit, ROE, growth, book value, operating cash flow per share, and gross margin retain only periods whose notice and update dates pass the completed cutoff. The source remains one third-party normalized dataset, ETFs are unsupported, and the assistant role is not wired to it. |
...
15. Point-in-time fundamentals require both disclosure and update dates to pass the completed cutoff, remain stock-only, and are not yet assistant inputs.
