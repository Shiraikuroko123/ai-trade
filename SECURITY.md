...
AI Trade is a research and local paper-trading tool. It has no bundled working live-broker adapter. The native workstation binds only to a loopback address, validates the Host header, and denies cross-origin writes. Beta mode stores only PBKDF2 password verifiers in the Git-ignored `state/` directory, keeps sessions in memory, uses `HttpOnly`/`SameSite=Strict` cookies, and binds every write token to the authenticated session. `serve --owner-local` deliberately bypasses beta login for the owner on a trusted local machine; it must never be treated as remote access control.

The source Docker deployment has one narrow exception for container networking. A wildcard container-interface bind is accepted only with the explicit `--container-bind` flag and beta authentication enabled; combining it with `--owner-local` fails closed. The supplied Compose file publishes the port only on host loopback, while application Host validation still accepts only `localhost` or loopback IPs. Changing that mapping to a public interface, adding a tunnel, or placing the service behind an Internet-facing proxy is unsupported: there is no TLS termination, centralized identity service, or remote session revocation. See [Docker deployment](docs/DOCKER_DEPLOYMENT.md).
